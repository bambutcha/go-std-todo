...
Для изменения порта создайте `.env` файл с `PORT=8080`, и docker-compose автоматически использует его.
...
│   ├── config
│   │   └── config.go - реализация кастомного парсинга конфигурации
│   ├── handler
│   │   ├── handler.go - обработчики запросов
│   │   └── handler_test.go - тесты обработчиков
│   └── todo
│       ├── todo.go - бизнес-логика CRUD операций
│       └── todo_test.go - тесты CRUD операций
├── .env.example - пример файла конфигурации с переменными окружения
├── .gitignore
├── Dockerfile
├── README.md
├── docker-compose.yml 
├── go.mod
└── task.md - задание вакансии
```
